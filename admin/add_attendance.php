		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					...
					//
					$sql = "INSERT INTO attendance (employee_id, date, time_in, status, time_in_img) VALUES ('$id', '$date_now', '$lognow', '$logstatus', '$image')";
					if($conn->query($sql)){
				...
				if($query->num_rows < 1){
					...
					if($row['time_out'] != '00:00:00'){
						...
						$output['message'] = 'You have timed out for today';
							...
							$time = date('H:i:s');
						$sql = "UPDATE attendance SET time_out = '$time' WHERE id = '".$row['uid']."'";
						if($conn->query($sql)){
